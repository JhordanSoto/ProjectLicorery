...
{
    public class SaleImplement : ISale
    {
        string query = "";
        public int Delete(ProductSale generic)
        {
            query = @"UPDATE Product SET quantity = @Quantity WHERE idProduct = @idProduct; ";
            try
            {
                MySqlCommand cmd = DBImplementation.CreateBasicCommand(query);
                cmd.Parameters.AddWithValue("@Quantity", generic.newQuantity);
                cmd.Parameters.AddWithValue("@idProduct", generic.nro);
        ...
        public int Insert(Sale generic)
        {
            query = "INSERT INTO Sale (Ci,IdUser,IdProduct,QuantityOfProducts,Import) VALUES (@Ci,@IdUser,@IdProduct,@QuantityOfProducts,@Import)";
            try
            {
                MySqlCommand cmd = DBImplementation.CreateBasicCommand(query);
                cmd.Parameters.AddWithValue("@Ci", generic.ci);
                cmd.Parameters.AddWithValue("@IdUser", generic.idUser);
                cmd.Parameters.AddWithValue("@IdProduct", generic.idProduct);
                cmd.Parameters.AddWithValue("@QuantityOfProducts", generic.QuantityOfProducts);
                cmd.Parameters.AddWithValue("@Import", generic.Import);
                return DBImplementation.ExecuteBasicCommand(cmd);
            }
            catch (Exception ex)
            {
